...
buymeacoffee.com/MilkDrop2077

update v3.0.0 2021/03/28 :
-50% more presets included (100 extra presets added for a total of 300!)
...
type
  ...
  TForm1 = class(TForm)
    ...
    Panel1: TPanel;
    Timer1: TTimer;
    procedure CheckBox1RandColorChange(Sender: TObject);
    procedure CheckBox1RandColorMouseMove(Sender: TObject; Shift: TShiftState; X,
      Y: Integer);
    procedure CheckBox2ForceRandColorChange(Sender: TObject);
    procedure CheckBox3MashOFFChange(Sender: TObject);
    procedure CheckBox3MashOFFClick(Sender: TObject);
    procedure CheckBox3MashOFFMouseMove(Sender: TObject; Shift: TShiftState; X,
      Y: Integer);
    procedure CheckBox4MashALLChange(Sender: TObject);
    procedure CheckBox4MashWCChange(Sender: TObject);
    ...
    procedure CheckBox4MashALLMouseMove(Sender: TObject; Shift: TShiftState; X,
      Y: Integer);
    procedure CheckBox5AddEffectChange(Sender: TObject);
    procedure CheckBox6ChangeBitsChange(Sender: TObject);
    ...
    procedure CheckBox2ForceRandColorMouseMove(Sender: TObject; Shift: TShiftState; X,
      Y: Integer);
    procedure CheckBox7RandALLChange(Sender: TObject);
...
var
  ...
  x, XXXX, RANDOMZ, L, LWC, total, total2, totalBits, lastt, lasttt : integer;
  ...
  numberz2,numberz3 : tstringlist;

  md :boolean;
  x0, y0 :integer;
  PrevWndProc: WNDPROC;

  XXXChance, XXXValue : integer;
  RandColor, ForceRandColor, MashOFF, MashWC, MashALL, AddEffect, ChangeBits, RandALL :boolean;
...
procedure CHANGEBITSOFCODE : Change bits of Code : replace a 'cos' with a 'tan' ect
...
for j := i to numberz.count - 1 do begin ListMilk.Delete(I); end; // delet line
ListMilk.Add('comp_'+s+'=`'+ForceFLOT3[RandomRange(1,4)]);       // 1 to 3 (4 not included), see const. on top
ListMilk.Add('comp_'+inttostr(strtoint(s)+1)+'=`}');          // add new line } with comp #+1
found := true;
end;
end;
...
// Also load the 300 presets for autogeneration
...
// Change bits of Code : replace a 'cos' with a 'tan' ect
procedure CHANGEBITSOFCODE;
var
BitsofCode : string;
i,bpos : integer;
Bits_cos, Bits_sin, Bits_time, Bits_y, Bits_abs : boolean;
begin
...
bpos := round(ListMilk.count/2); // to stop half way, first part of milk file not interesting to change bits of code

for i := ListMilk.count - 1 downto bpos do begin // start from bottom to middle

  if not Bits_cos then begin
  if Pos('cos', ListMilk[i])<>0 then begin
    BitsofCode := NewStringReplace(ListMilk[i], 'cos', 'tan',[rfIgnoreCase]); // no rfReplaceAll to replace only once
    ListMilk[i] := BitsofCode;
    Bits_cos := true; // if replaced, then skip
    totalBits := totalBits+1;
  end; end;

  if not Bits_sin then begin
  if Pos('sin', ListMilk[i])<>0 then begin
    BitsofCode := NewStringReplace(ListMilk[i], 'sin', 'tan',[rfIgnoreCase]);
    ListMilk[i] := BitsofCode;
    Bits_sin := true;
    totalBits := totalBits+1;
  end; end;

  if not Bits_time then begin
  if Pos('time', ListMilk[i])<>0 then begin
    BitsofCode := NewStringReplace(ListMilk[i], 'time', 'ang',[rfIgnoreCase]);
    ListMilk[i] := BitsofCode;
    Bits_time := true;
    totalBits := totalBits+1;
  end; end;

  if not Bits_y then begin
  if Pos('.y', ListMilk[i])<>0 then begin
    BitsofCode := NewStringReplace(ListMilk[i], '.y', '.x',[rfIgnoreCase]);
    ListMilk[i] := BitsofCode;
    Bits_y := true;
    totalBits := totalBits+1;
  end; end;
...
//Mashup Warp code + Comp code ONLY
//R = 0 to 299, perfect for PRESETS.RES, MILKD0 to MILKD299
...
XXXChance := strtoint(LabelChance.Caption);
XXXValue := strtoint(LabelValue.Caption);
...
if ImgOn1.Visible = true then begin
    ...
    //Random ALL
    if RandALL then begin LVLRandomALL; sleep(1);
    ListMilk3.Add('RANDOMIZE EVERYTHING : ON'); end;


    //NO MASHUP
    if MashOFF then begin
    R := Random(300); // = Random 0 to 299 to randomly load one of the 300 presets
    sleep(1); // to avoid identical 'R' number
    ListMilk3.Add('Original Preset : MilkDrop2077.R'+FormatFloat('000', R));
    LoadStringListFromResource('MILK'+inttostr(R),ListMilk); end;

    //Warp+Composite
    if MashWC then begin AUTOGEN_MILKRANDOM; end;

    //Frame+Pixel+Warp+Composite
    if MashALL then begin AUTOGEN_MILKRANDOMALL; end;


     //CODE CHANGE
    if XXXChance <> 0 then begin
    ListMilk3.Add('Level of RANDOMIZATION >> Around '+inttostr(XXXChance)+'% of editable values were changed up to - or + '+inttostr(XXXValue)+'%');
    numberz3 := ListMilk;
    for i := 0 to ListMilk.Count-1 do begin
    if Pos('fWaveAlpha=', ListMilk[i])<>0 then start := i; end;
    if start = 0 then start := 35;
    MILKDROP(ListMilk, XXXChance, XXXValue, start);
    ListMilk3.Add('Result : A total of '+inttostr(lastt)+' out of '+inttostr(lasttt)+' values have been changed :)');
    end;


    //COLOR CHANGE
    if RandColor then begin MILKFLOAT3(ListMilk); ListMilk3.Add('Randomize colors : ON'); end;
    ...
    ADDMILKEFFECT(ListMilk,addmilk.Strings[z]);
    ListMilk3.Add('Extra effect added : '+addmilk.Strings[z]);
    end;

    ListMilk3.Add('################################################################################');
    if not MashOFF then begin
    ...
    ListMilk3.Add(ListMilk.Text);//add mashup
...
if ImgOn2.Visible = true then begin
    ...
    //Warp+Composite
    if MashWC then begin MILKRANDOM; end;

    //Frame+Pixel+Warp+Composite
    if MashALL then begin MILKRANDOMALL; end; //ListMilk = MASHED CODE
